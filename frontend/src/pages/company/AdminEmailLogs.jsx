...
function AdminEmailLogs() {
  ...
  const fetchLogs = async () => {
    try {
      const res = await API.get("/company/emails");
      setLogs(res.data);
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="dashboard-layout">
      <aside className="dashboard-sidebar">
        ...
        <nav className="nav-menu">
          ...
          <Link to="/company/scans" className="nav-link">🔍 Resume Scans</Link>
          <Link to="/company/emails" className="nav-link active">📧 Email Logs</Link>
