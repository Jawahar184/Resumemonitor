...
def get_dashboard_stats(request: Request):
    ...
    open_vacancies  = db.jobs.count_documents({"posted_by": company_key})
    resumes_scanned = db.resumes.count_documents({})
    top_matches     = db.matches.count_documents({"posted_by": company_key, "match_score": {"$gte": 70}})
    alerts_sent     = db.matches.count_documents({"posted_by": company_key, "status": "Email Sent"})
...
def get_eligible_candidates(request: Request):
    """Return all matched candidates for this company's jobs, newest first."""
    company_key = get_company_key(request)
    candidates = list(db.matches.find({"posted_by": company_key}, {"_id": 0}))
